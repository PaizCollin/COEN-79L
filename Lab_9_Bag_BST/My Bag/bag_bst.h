...
#pragma mark - Member Function Implementation
    ...
    //returns the count of a certain value in the tree
    template <class Item>
    ...
    //erases only the first appearance of the target in the tree
    template <class Item>
    ...
    //appends the tree to the tree that activated this func
    template <class Item>
    ...
    //adds two trees together and returns it
    template <class Item>
    bag<Item> operator +(const bag<Item>& b1, const bag<Item>& b2)
    {
        // STUDENT WORK
    ...
    //inserts all nodes of a tree into the one that activated this
    template <class Item>
